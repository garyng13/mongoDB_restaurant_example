...
.post(authenticate.verifyUser,(req, res, next)=>{
    res.statusCode = 404;
    res.end('Post operation is not supported on' + req.params.dishId + '/comment/' 
    +req.comments.commentId);
})
...
.put(authenticate.verifyUser,(req,res,next)=>{
    ...
    .then((dish)=>{
        if(dish !=null && req.params.commentId !=null){
             if(req.body.rating){dish.comments.rating = req.body.rating};
             if(req.body.comment){dish.comments.comment = req.body.comment};
             dish.save()
             .then((dish)=>{
                 res.statusCode = 200;
                 res.setHeader('Content-Type', 'application/json');
